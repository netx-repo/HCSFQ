...
def analyze_throughput():
    with open(run_folder_path + '/flow_throughput.csv.log') as file:
        reader = csv.reader(file)

        print("Reading in throughput log file...")
        ...
        for row in reader:
            ...
            source_ids.append(float(row[1]))
            target_ids.append(float(row[2]))
            amount_bytes.append(float(row[3]))
            abs_start_time_ns.append(float(row[4]))
            abs_end_time_ns.append(float(row[5]))
            if len(row) != 6:
                print("Invalid row: ", row)
                exit()
        print("Calculating statistics...")
