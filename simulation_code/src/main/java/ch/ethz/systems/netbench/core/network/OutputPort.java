...
public abstract class OutputPort {
    ...
    public int[] A;
    public Queue  buffQueue, buffQueue2;
    public int enqueueMode;
    public int numRanks;
    /**
     * Constructor.
     *
    ...
    protected OutputPort(NetworkDevice ownNetworkDevice, NetworkDevice targetNetworkDevice, Link link, Queue<Packet> queue) {
        ...
        // this.numRanks = 100000010;
        // this.A = new int[this.numRanks];
        this.buffQueue = new LinkedList();
        this.buffQueue2 = new LinkedList();
        this.enqueueMode = 1;
    }
    ...
    protected final void guaranteedEnqueue(Packet packet) {
        ...
        if (!isSending) {
            ...
            // if (this.enqueueMode == 1) {
            //     queue.add(packet);
            //     buffQueue.add(packet);
            // }
            // else {
            //     // TODO: CHANGE
            //     buffQueue2.add(packet);
            // }
            logger.logQueueState(queue.size(), bufferOccupiedBits);
        }
    }
    ...
    void dispatch(Packet packet) {
        ...
        if (!queue.isEmpty()) {
            // TODO: CHANGE
        // if ((!buffQueue.isEmpty()) || (!buffQueue2.isEmpty())) {

            // Pop from queue
            buffQueue.poll();
            Packet packetFromQueue = queue.poll();
            // Packet packetFromQueue;
            // if (buffQueue.size() > 0) {
            //     buffQueue.poll();
            //     packetFromQueue = queue.poll();
            // }
            // else {
            //     // buffQueue2.poll();
            //     packetFromQueue = (Packet) buffQueue2.poll();
            // }
            ...
            //**  algorithm 3: TODO: change for other methods */
            // PriorityHeader header = (PriorityHeader) packet;
            // decreaseA((int) header.getPriority());
