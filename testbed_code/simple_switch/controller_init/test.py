...
from simpleswitch.p4_pd_rpc.ttypes import *
...
def scapy_simpleswitch_bindings():
...
def init_tables(test, sess_hdl, dev_tgt):
    ...
    for i in range(len(ipv4_table_address_list)):
        match_spec = simpleswitch_ipv4_route_match_spec_t(ipv4_table_address_list[i])
        action_spec = simpleswitch_set_egress_action_spec_t(ipv4_table_port_list[i])
    ...
    # test.client.mac_route_set_default_action__drop(sess_hdl, dev_tgt)
    # for i in range(len(ethernet_set_mac_dst)):
    #     match_spec = simpleswitch_mac_route_match_spec_t(ethernet_set_mac_dst[i])
    #     action_spec = simpleswitch_set_egress_action_spec_t(ipv4_table_port_list[i])
    #     entry_hdl = test.client.mac_route_table_add_with_set_egress(
    #         sess_hdl, dev_tgt, match_spec, action_spec)
...
class SimpleTest(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["simpleswitch"])
        scapy_simpleswitch_bindings()
