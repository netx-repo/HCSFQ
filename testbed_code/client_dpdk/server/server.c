...
static void process_packet_client(uint32_t lcore_id, struct rte_mbuf *mbuf) {
    ...
    tput_stat[stat_idx].rx += (sizeof(MessageHeader) + sizeof(struct udp_hdr) + sizeof(struct ipv4_hdr) + sizeof(struct ether_hdr)) * 8;

    // uint8_t stat_idx = ntohl(ip->src_addr) - 167837696;
    // // printf("stat_idx %d",stat_idx);
    // tput_stat[stat_idx].rx += 1;
    // // tput_stat[(ntohl(message_header->flow_id) / 10 - 1) * 3 + ntohl(message_header->flow_id) % 10].rx += 1;
    // // if (lcore_id == 0) {
    //     if (tot_count[stat_idx] == 10000) {
    //         tot_count[stat_idx] = 0;
    //         // printf("idx:%d, label:%d, congested:%d, per_tenant_F:%d, total_F:%d, per_tenant_alpha:%d, total_alpha:%d, to_drop:%d, delta_total_alpha:%d\n", stat_idx, ntohl(message_header->label), message_header->congested, ntohl(message_header->per_tenant_F), ntohl(message_header->total_F), ntohl(message_header->per_tenant_alpha), ntohl(message_header->total_alpha), message_header->to_drop, ntohl(message_header->delta_total_alpha));
    //     }
    //     tot_count[stat_idx] += 1;
    // // }
}
    ...
    while (1) {
        ...
        if ((lcore_id == rte_get_master_lcore()) && (update_tsc > 0)) {
            ...
            if (unlikely(average_end_tsc > 0 && cur_tsc > average_end_tsc)) {
                ...
                // printf("Throughput per second:\n");
                // for (i = 0; i < second; ++i)
                //     // printf("%d: %d\n", i, throughput_per_second[i]);
