...
static void generate_request_pkt(uint32_t lcore_id, struct rte_mbuf *mbuf, uint64_t pkts_send_ms) {
    ...
    // key
    message_header->recirc_flag = (uint8_t) 0;
    message_header->tenant_id = htons(tenant_idx);
    message_header->flow_id = htons(client_idx);
    // ** estimate the speed at endhost
    message_header->label = htonl((uint32_t)((float)(pkts_send_ms) * 61.4)) ;

    message_header->congested = 0;
    message_header->per_tenant_F = 0;
    message_header->total_F = 0;
    message_header->per_tenant_alpha = 0;
    message_header->total_alpha = 0;
    message_header->delta_total_alpha = 0;
    message_header->to_drop = 0;
    message_header->pertenantF_leq_totalalpha = 0;
    // if (lcore_id <= 3) {
    //     message_header->flow_id = htonl(client_idx * 10);
    // } else if (lcore_id <=6) {
    //     message_header->flow_id = htonl(client_idx * 10 + 1);
    // } else {
    //     message_header->flow_id = htonl(client_idx * 10 + 2);
    // }
    // message_header->label = htonl(0);
}
...
static int32_t nc_test_tx_loop(uint32_t lcore_id) {
    ...
    uint64_t next_s_tsc = cur_tsc + update_tsc;
    ...
    while (1) {
        ...
        if (unlikely(cur_tsc > next_s_tsc)) {
            pkts_send_limit_ms = pkts_send_limit_ms + 200;
            next_s_tsc += s_tsc;
        }
